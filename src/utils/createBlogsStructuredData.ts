...
export default function createBlogsStructuredData(
  info: BlogEntryRaw
): createStructuredDataReturn {
  const dateSplitted: Array<string> = info.publicationDate?.split('-') as any;
  ...
  const dateFormated = blogDate.toISOString();
  const structuredData = {
    '@context': 'https://schema.org',
    '@type': 'Article',
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `https://ulisessg.com/${info.name}`,
    },
    headline: `${info.title} | UlisesSG`,
    datePublished: dateFormated,
    dateModified: info.lastUpdate
